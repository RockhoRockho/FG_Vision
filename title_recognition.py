...
def title_recognition():
    ...
    def load_model():
        ...
        saved_model = './saved_model/best_loss_0.35_model.h5'
    ...
    #양식 갯수 만큼 돌리기
    #돌면서 json에 있는 좌표값들을 사용자의 이미지에 대입해서 양식의 제목과 일치하는것을 찾기
    #만약에 해당 좌표에 인식할 문자가 아예 없으면 에러가 떠서 try except로 진행
    for i in range(len(json_data)):
        try:
            w = json_data[i]['lot'][0]['w']
            h = json_data[i]['lot'][0]['h']
            cx = json_data[i]['lot'][0]['cx']
            cy = json_data[i]['lot'][0]['cy']
            ...
            for j in boxes:
                ...
                img_cropped = cv2.getRectSubPix(
                        bbb,
                        patchSize=((j[1][0] - j[0][0]+2), (j[1][1] - j[0][1])+2),
                ...
                result = tf.argmax(y)
                word += (idx2char[result])
            #word는 인식된 제목 
            #json에 있는 인식 잘되는 제목의 부분과 word를 비교해서 양식 판별
            if word == json_data[i]['lot'][1]['title']:
                #i번째 양식 (제목 보다 이게 더 편해서 이걸로 잡음)
                form = i
                print(form)
                break
        except:
